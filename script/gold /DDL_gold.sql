...
SELECT 
      ...
      ,CASE WHEN ci.cst_gndr != 'n/a' THEN ci.cst_gndr --CRM is the Master for Gender in this case 
            ELSE COALESCE(ca.gen, 'n/a')
       END AS gender
      ,ca.bdate AS birth_date
      ,ci.[cst_create_date] AS create_date

  FROM [master].[silver].[crm_cust_info] ci
  LEFT JOIN master.silver.erp_cust_az12 ca
  ON        ci.cst_key = ca.cid
  LEFT JOIN master.silver.erp_loc_a101 la 
...
FROM [master].[silver].[crm_prd_info] pn
LEFT JOIN master.silver.erp_px_cat_g1v2 pc
ON        pn.cat_id = pc.id
WHERE prd_end_dt IS NULL      --Filter out all old historical data
...
SELECT sd.[sls_ord_num] AS order_number
      ...
      ,sd.[dwh_create_date]
FROM [master].[silver].[crm_sales_details] sd
